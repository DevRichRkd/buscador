            ...
            <h1>{{__('Temas')}}</h1>
                <div class="row">  
                    @if($perm['R'])
                        @if($perm['I'])
                            <div class="col-md-12">
                                <a href="{{ route('temas.create') }}" class="float-right" data-toggle="tooltip" data-placement="left" title="" data-original-title="{{__('clickAgregar')}} " data-custom-class="tooltip-dark">
                                  ...
                                  <tbody>
                                    ...
                                    @foreach($temas as $valor)
                                        <tr>
                                            ...
                                            @include("layouts.partials.td-actualizar-eliminar",['id' => $valor->id, 'nombre' => $valor->nombre, 'recurso' => 'temas'])                                 
                                        </tr>
                                        @php $count++; @endphp
                                    @endforeach
                                  </tbody>
