...
class GlosarioController extends Controller{
    ...
    public function index(){
        ...
        $glosario =  DB::connection('mysql')->select("SELECT * FROM glosario");
        return view('glosario/listar')->with([
                                                'glosario' => $glosario,
                                                'perm'     => $perm[0],
                                            ]);
    }
    ...
    public function create(){
        ...
        return view('glosario/registrar')->with([
                                                'perm'     => $perm[0],
                                                ]);
    }
    ...
    public function store(Request $request){
        ...
        $this->validate($request, [
            'nombre'      => 'required|unique:glosario|max:50',
            'descripcion' => 'required|min:10|max:120'
        ]);
        ...
        $glosario->nombre      = $request['nombre'];
        $glosario->descripcion = $request['descripcion'];
        $glosario->estatus     = $estatus;

        if ($glosario->save()) {
            return redirect('/glosario')->with([
    ...
    public function edit($id){
        ...
        $glosario = Glosario::findOrFail($id);

        return view('glosario/modificar')->with([
                                                    "glosario" => $glosario,
                                                    "perm"     => $perm[0]
                                                ]);
    }
    ...
    public function update(Request $request, Glosario $glosario){
        ...
        if ($request->nombre != $request->nombreOriginal) {
            $this->validate($request, [
                'nombre'      => 'required|unique:glosario,nombre,'.$glosario->id,
                'descripcion' => 'required|min:10|max:120'
            ]);
        }else{
             $this->validate($request, [
    ...
    public function destroy($id){
        ...
        $existe = $this->existenDependientes('id_glosario',$id);
        if (count($existe) >= 1) {
